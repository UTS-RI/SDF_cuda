...
inline tinyply::Type property_type_from_string(const std::string & t)
{
    if (t == "int8" || t == "char")           return tinyply::Type::INT8;
    else if (t == "uint8" || t == "uchar")    return tinyply::Type::UINT8;
    else if (t == "int16" || t == "short")    return tinyply::Type::INT16;
    else if (t == "uint16" || t == "ushort")  return tinyply::Type::UINT16;
    else if (t == "int32" || t == "int")      return tinyply::Type::INT32;
    else if (t == "uint32" || t == "uint")    return tinyply::Type::UINT32;
    else if (t == "float32" || t == "float")  return tinyply::Type::FLOAT32;
    else if (t == "float64" || t == "double") return tinyply::Type::FLOAT64;
    return tinyply::Type::INVALID;
}

typedef std::function<void(void * dest, const char * src, bool be)> cast_t;

struct tinyply::PlyFile::PlyFileImpl
{
...
tinyply::PlyProperty::PlyProperty(std::istream & is) : isList(false)
{
    std::string type;
    is >> type;
    if (type == "list")
...
tinyply::PlyElement::PlyElement(std::istream & is)
{
    is >> name >> size;
}
...
int64_t find_element(const std::string & key, const std::vector<tinyply::PlyElement> & list)
{
    for (size_t i = 0; i < list.size(); i++) if (list[i].name == key) return i;
    return -1;
}

int64_t find_property(const std::string & key, const std::vector<tinyply::PlyProperty> & list)
{
    for (size_t i = 0; i < list.size(); ++i) if (list[i].name == key) return i;
    return -1;
}

bool tinyply::PlyFile::PlyFileImpl::parse_header(std::istream & is)
{
    std::string line;
...
void tinyply::PlyFile::PlyFileImpl::read_header_text(std::string line, std::istream & is, std::vector<std::string>& place, int erase)
{
    place.push_back((erase > 0) ? line.erase(0, erase) : line);
}

void tinyply::PlyFile::PlyFileImpl::read_header_format(std::istream & is)
{
    std::string s;
    (is >> s);
...
void tinyply::PlyFile::PlyFileImpl::read_header_element(std::istream & is)
{
    elements.emplace_back(is);
}

void tinyply::PlyFile::PlyFileImpl::read_header_property(std::istream & is)
{
    if (!elements.size()) throw std::runtime_error("no elements defined; file is malformed");
    elements.back().properties.emplace_back(is);
}

size_t tinyply::PlyFile::PlyFileImpl::read_property_binary(const tinyply::Type & t, const size_t & stride, void * dest, size_t & destOffset, std::istream & is)
{
    destOffset += stride;
...
size_t tinyply::PlyFile::PlyFileImpl::read_property_ascii(const tinyply::Type & t, const size_t & stride, void * dest, size_t & destOffset, std::istream & is)
{
    destOffset += stride;
    switch (t)
    {
    case tinyply::Type::INT8:       *((int8_t *)dest) = ply_read_ascii<int32_t>(is);   break;
    case tinyply::Type::UINT8:      *((uint8_t *)dest) = ply_read_ascii<uint32_t>(is); break;
    case tinyply::Type::INT16:      ply_cast_ascii<int16_t>(dest, is);                 break;
    case tinyply::Type::UINT16:     ply_cast_ascii<uint16_t>(dest, is);                break;
    case tinyply::Type::INT32:      ply_cast_ascii<int32_t>(dest, is);                 break;
    case tinyply::Type::UINT32:     ply_cast_ascii<uint32_t>(dest, is);                break;
    case tinyply::Type::FLOAT32:    ply_cast_ascii<float>(dest, is);                   break;
    case tinyply::Type::FLOAT64:    ply_cast_ascii<double>(dest, is);                  break;
    case tinyply::Type::INVALID:    throw std::invalid_argument("invalid ply property"); 
    }
    return stride;
}

void tinyply::PlyFile::PlyFileImpl::write_property_ascii(Type t, std::ostream & os, uint8_t * src, size_t & srcOffset)
{
    switch (t)
    {
...
void tinyply::PlyFile::PlyFileImpl::write_property_binary(Type t, std::ostream & os, uint8_t * src, size_t & srcOffset, const size_t & stride)
{
    os.write((char *)src, stride);
    srcOffset += stride;
}

void tinyply::PlyFile::PlyFileImpl::read(std::istream & is)
{
...
void tinyply::PlyFile::PlyFileImpl::write(std::ostream & os, bool _isBinary)
{
	// reset cursors
...
void tinyply::PlyFile::PlyFileImpl::write_binary_internal(std::ostream & os)
{
    isBinary = true;
    write_header(os);
...
void tinyply::PlyFile::PlyFileImpl::write_ascii_internal(std::ostream & os)
{
    write_header(os);
...
void tinyply::PlyFile::PlyFileImpl::write_header(std::ostream & os)
{
...
std::shared_ptr<tinyply::PlyData> tinyply::PlyFile::PlyFileImpl::request_properties_from_element(const std::string & elementKey,
    const std::initializer_list<std::string> propertyKeys,
    const uint32_t list_size_hint)
{
...
void tinyply::PlyFile::PlyFileImpl::add_properties_to_element(const std::string & elementKey, 
    const std::initializer_list<std::string> propertyKeys, 
    const Type type, const size_t count, uint8_t * data, const Type listType, const size_t listCount)
{
    ParsingHelper helper;
...
void tinyply::PlyFile::PlyFileImpl::parse_data(std::istream & is, bool firstPass)
{
...
tinyply::PlyFile::PlyFile() { impl.reset(new PlyFileImpl()); };
tinyply::PlyFile::~PlyFile() { };
bool tinyply::PlyFile::parse_header(std::istream & is) { return impl->parse_header(is); }
void tinyply::PlyFile::read(std::istream & is) { return impl->read(is); }
void tinyply::PlyFile::write(std::ostream & os, bool isBinary) { return impl->write(os, isBinary); }
std::vector<tinyply::PlyElement> tinyply::PlyFile::get_elements() const { return impl->elements; }
std::vector<std::string> & tinyply::PlyFile::get_comments() { return impl->comments; }
std::vector<std::string> tinyply::PlyFile::get_info() const { return impl->objInfo; }
std::shared_ptr<tinyply::PlyData> tinyply::PlyFile::request_properties_from_element(const std::string & elementKey,
    const std::initializer_list<std::string> propertyKeys,
    const uint32_t list_size_hint)
{
